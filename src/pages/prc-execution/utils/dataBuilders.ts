		...
		if (formData.entries && Array.isArray(formData.entries)) {
			formData.entries.forEach(entry => {
				...
				simplifiedData[materialId][order.toString()] = {
					...
					hygrometerInstrumentId: entry.hygrometerInstrumentId ?? '',
					weighingMachineInstrumentId: entry.weighingMachineInstrumentId ?? '',
					actualQuantity: entry.actualQuantity,
					fodCheckpoint: entry.fodCheckpoint ?? '',
					fodDeviationComment: entry.fodDeviationComment ?? '',
