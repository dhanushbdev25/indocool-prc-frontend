...
const StepSelectionCard = ({ item, onClick, isSelected }: StepSelectionCardProps) => {
	...
	const getItemType = () => {
		return isSequence ? 'sequence' : 'inspection';
	};
			...
			<CardContent sx={{ p: 2 }}>
				<Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', mb: 1 }}>
					<Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
						<Box sx={{ color: getTypeColor(), display: 'flex', alignItems: 'center' }}>
							{isSequence ? <SequenceIcon /> : <InspectionIcon />}
						</Box>
						<Chip
				...
				<Typography variant="h6" sx={{ fontSize: '1rem', fontWeight: 600, color: '#333', mb: 0.5, lineHeight: 1.3 }}>
					{getItemName()}
				</Typography>
				...
				<Typography variant="caption" sx={{ color: '#999', fontSize: '0.75rem', display: 'block', mt: 1 }}>
