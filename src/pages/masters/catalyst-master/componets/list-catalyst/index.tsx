...
import { useFetchCatalystChartQuery } from '../../../../../store/api/business/catalyst-master/catalyst.api';
...
const ListCatalyst = () => {
    ...
    // Fetch catalyst chart data using the API with Zod validation
    const { 
        data: catalystChartData, 
        isLoading: isCatalystDataLoading
    } = useFetchCatalystChartQuery({ id: 1 });
    ...
    const handleEdit = (catalystId: number) => {
        navigate(`/editcatalyst/${catalystId}`);
    };
    ...
    return (
        <Box sx={{ p: 3, backgroundColor: '#f5f5f5', minHeight: '100vh' }}>
            <CatalystHeader />
            {catalystChartData && (
                <SummaryCards headerData={catalystChartData.header} />
            )}
            <ChartManagement 
