...
import { Search as SearchIcon, Checklist as ChecklistIcon } from '@mui/icons-material';
...
const InspectionManagement = ({ onSearchChange, onFilterChange }: InspectionManagementProps) => {
	const [activeFilter, setActiveFilter] = useState('All Inspections');
	const [searchTerm, setSearchTerm] = useState('');

	const filterButtons = ['All Inspections', 'ACTIVE', 'NEW', 'INACTIVE'];
		...
		<Box sx={{ backgroundColor: 'white', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
			...
			<Box sx={{ p: 3, borderBottom: '1px solid #e0e0e0' }}>
				<Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
					<ChecklistIcon sx={{ color: '#666', mr: 1, fontSize: '1.25rem' }} />
					<Typography
						variant="h5"
						sx={{
							fontWeight: 600,
			...
			<Box sx={{ p: 3, display: 'flex', gap: 2, alignItems: 'center' }}>
				<TextField
					placeholder="Search by Inspection ID, name or type"
					variant="outlined"
					size="small"
					value={searchTerm}
